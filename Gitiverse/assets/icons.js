/* ---------------------------------------------------------------------------
 * icons.js — Lucide icon sprite (inline, no network required)
 *
 * Icon artwork from Lucide v1.46.0 — https://lucide.dev/icons/ — ISC License.
 * Path data is inlined verbatim so the prototype also works offline / from file://.
 *
 * Public API (attached to window.GitIcons):
 *   GitIcons.svg(name, opts)   -> SVG markup string
 *   GitIcons.has(name)         -> boolean
 *   GitIcons.hydrate(root)     -> replace [data-icon] placeholders inside root
 * ------------------------------------------------------------------------- */
...
(function (global) {
  ...
  /**
   * Build an <svg> string for a Lucide icon.
   * @param {string} name  lucide icon name, e.g. 'git-branch'
   * @param {{size?:number, cls?:string, stroke?:number, style?:string}} [opts]
   * @returns {string} SVG markup (empty string when the icon is unknown)
   */
  function svg(name, opts) {
    ...
    var cls = 'lucide lucide-' + name + (opts.cls ? ' ' + opts.cls : '');
    return (
  ...
  /** @returns {boolean} whether the icon exists in the sprite */
  function has(name) { return Object.prototype.hasOwnProperty.call(PATHS, name); }

  /** @returns {string[]} every icon name bundled in the sprite */
  function names() { return Object.keys(PATHS); }

  /**
   * Replace every <i data-icon="name" data-size=".." class=".."> inside root
   * with the matching inline SVG. Idempotent.
   */
  function hydrate(root) {
    var scope = root || global.document;
    if (!scope || !scope.querySelectorAll) return;
    var slots = scope.querySelectorAll('[data-icon]');
    for (var i = 0; i < slots.length; i++) {
      var el = slots[i];
      var markup = svg(el.getAttribute('data-icon'), {
        size: parseInt(el.getAttribute('data-size'), 10) || 24,
        cls: el.getAttribute('data-class') || '',
        stroke: parseFloat(el.getAttribute('data-stroke')) || 2
      });
      if (markup) el.outerHTML = markup;
    }
  }

  global.GitIcons = { svg: svg, has: has, names: names, hydrate: hydrate, PATHS: PATHS };
})(window);
